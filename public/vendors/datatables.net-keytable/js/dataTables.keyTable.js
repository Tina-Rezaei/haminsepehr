...
$.extend( KeyTable.prototype, {
	...
	focused: function ( cell )
	{
		...
		var lastIdx = this.s.lastFocus.cell.server();
	...
	_blur: function ()
	{
		...
		this._updateFixedColumns(cell.server().column);
	...
	_editor: function ( key, orig )
	{
		...
		editor
			...
			.inline( this.s.lastFocus.cell.server() );
	},
	...
	_focus: function ( row, column, shift, originalEvent )
	{
		...
		this.s.lastFocus = {
			...
			node: cell.node(),
			relative: {
				row: dt.rows( { page: 'current' } ).indexes().indexOf( cell.server().row ),
				column: cell.server().column
			}
		};
	...
	_shift: function ( e, direction, keyBlurable )
	{
		...
		var currRow = dt
			.rows( { filter: 'applied', order: 'applied' } )
			.indexes()
			.indexOf( currentCell.server().row );
		...
		var currCol = dt
			.columns( columns )
			.indexes()
			.indexOf( currentCell.server().column );
